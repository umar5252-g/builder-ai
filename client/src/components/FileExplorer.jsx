...
function TreeItem({ node, activeFile, onFileSelect, depth = 0 }) {
  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-3 py-1.5 px-2 text-xs transition-colors rounded-md cursor-pointer ${isActive ? "bg-zinc-100 text-zinc-950 font-medium" : "text-zinc-500 hover:bg-zinc-50 hover:text-zinc-900"}`}
      style={{ paddingLeft: `${depth * 12 + 8}px` }}
    >
...
const FileExplorer = ({ files, activeFile, onFileSelect }) => {
  ...
  return (
    <div className="h-full overflow-y-auto hide-scrollbar py-2">
